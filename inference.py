...
#-------------------------------------v11---------------------------------------
def detecty11(img_path, model_name='yolov11n.pt', detec_num=0):
    local_supply = {
    'Apple':    0,
    'Chapathi': 0,
    'Chicken Gravy':    0,
    ...
    for el in local_supply:
        supplyv11[el] = (min(supplyv11[el][0], local_supply[el]), max(supplyv11[el][1], local_supply[el]))

def detecty11_save(model_path, i):
    #take photo
    img = take_photo(0)
    img_path = save_img(f'original{i}.jpg', img)
    #detec
    detecty11(img_path, model_name=model_path, detec_num=i)
    #save
    save('.', supplyv11)
#-------------------------------------------------------------------------------
...
def main(opt):
    ...
    elif 'v11' in model_path:
        for i in range(int(opt.nphoto)):
            detecty11_save(model_path, i)
